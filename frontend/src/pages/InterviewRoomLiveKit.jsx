...
import { useInterviewLiveKit } from "../hooks/useInterviewLiveKit";
...
import InterviewerThinking from "../components/InterviewerThinking";
...
const InterviewRoomLiveKitContent = ({ sessionId, onBack }) => {
  ...
  const {
    ...
    reconnectAttempt,
    sttFallbackActive,
  } = useInterviewLiveKit(sessionId, initialPhase, {
    addBanner,
    removeBanner,
    removeBannerByType,
    codeEditorRef,
          ...
          <div className="p-6 sm:p-8 max-w-3xl mx-auto w-full">
            ...
            {feedback ? (
              <FeedbackCard
                ...
                scores={
                  typeof feedback === "object" && feedback?.full?.scores
                    ? feedback.full.scores
                    : undefined
                }
              />
            ) : (
              <div className="flex flex-col items-center gap-4 py-8">
                <div
                  className="h-10 w-10 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin"
                  aria-hidden
                />
        ...
        <footer className="flex-shrink-0 p-6 border-t border-[var(--border-subtle)] bg-raised">
          <div className="max-w-3xl mx-auto w-full">
            ...
            >
              {feedback
                ? "Back to Dashboard"
                : "Back to Dashboard without waiting"}
            </button>
          </div>
        </footer>
      </div>
  ...
  return (
    <div className="h-screen flex flex-col bg-base overflow-hidden">
      {errorAnnouncer}
      {showFallbackButton && (
        <div className="flex-shrink-0 px-4 py-2 bg-amber-900/30 border-b border-amber-600/40 flex items-center justify-between gap-4">
          <p className="text-sm text-amber-200 truncate">{error}</p>
          <button
            type="button"
            onClick={fallbackToWebSocket}
            className="flex-shrink-0 px-3 py-1.5 rounded-lg bg-amber-600/80 text-white text-sm font-medium hover:bg-amber-500"
          >
      ...
      {sttFallbackActive && (
        <TextInputFallback
          onSubmit={(text) => sendControl({ type: "text_answer", text })}
        />
      )}
            ...
            >
              <div className="flex-1 flex flex-col items-center justify-center p-6 min-h-0">
                <div className="w-full max-w-2xl flex flex-col items-center">
                  ...
                  {status === "thinking" ? (
                    <div className="flex items-center gap-2 mt-1 text-sm text-cyan-300">
                      <InterviewerThinking />
                      <span>Thinking...</span>
                    </div>
                  ) : (
              ...
              />
              <div
                className="absolute left-1/2 -translate-x-1/2 text-center w-full max-w-[600px] px-4 pointer-events-none z-10"
                style={{ bottom: "calc(30% - 3rem)" }}
              >
