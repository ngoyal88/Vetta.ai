...
import { CODING_LANGUAGES } from "features/interview/coding/codeEditorUtils";
import type { CodingLanguage } from "features/interview/coding/codingQuestion";
import type { FeedbackPayload, OptionsRef } from "features/interview/types";
...
export const useSessionPersistence = (
  ...
  const restoreCodeToEditor = useCallback(
    (notify = true) => {
      ...
      if (
        typeof editor.setLanguage === "function" &&
        langBackup &&
        (CODING_LANGUAGES as readonly string[]).includes(langBackup)
      ) {
        editor.setLanguage(langBackup as CodingLanguage);
      }
      if (notify) {
      ...
      if (document.hidden) {
        ...
        if (syncSessionStatus) {
          void syncSessionStatus().then((result) => {
            ...
            if (result != null && typeof result === "object" && result.ended) {
